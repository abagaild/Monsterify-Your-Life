...
from core.trainer import get_trainers_from_database, get_all_trainers
from core.mon import get_mons_for_trainer
from core.items import get_inventory_quantity
from market.farm_breeding import breed_mons, OffspringRegistrationView, FarmResultView
...
class FarmInitialViewLogic(discord.ui.View):
    ...
    def __init__(self, user: discord.User):
        ...
        player_trainers = get_trainers_from_database(str(self.user.id))
    ...
    async def check_and_proceed(self, interaction: discord.Interaction):
        if self.player_trainer_id and self.all_trainer_id:
            ...
            # Use modern inventory function – note: get_inventory_quantity expects trainer id, category, and item name.
            total_leeway = get_inventory_quantity(player_trainer["character_name"],  "Legacy Leeway", "ITEMS")
            if total_leeway < 1:
                await interaction.followup.send(
                    f"Your trainer {player_trainer['character_name']} does not have enough Legacy Leeways to breed.",
                    ephemeral=True
                )
                return
...
class FarmMonSelectViewLogic(discord.ui.View):
    ...
    def __init__(self, user: discord.User, player_trainer: dict, all_trainer: dict):
        ...
        self.player_mon_map = {mon["mon_id"]: mon for mon in player_mons}
        self.all_mon_map = {mon["mon_id"]: mon for mon in all_mons}

        player_options = [discord.SelectOption(label=mon["name"], value=str(mon["mon_id"])) for mon in player_mons]
        if not player_options:
            player_options = [discord.SelectOption(label="No mons available", value="none", default=True)]
        all_options = [discord.SelectOption(label=mon["name"], value=str(mon["mon_id"])) for mon in all_mons]
        if not all_options:
    ...
    async def check_and_breed(self, interaction: discord.Interaction):
        if self.selected_player_mon is not None and self.selected_all_mon is not None:
            ...
            if not player_mon or not other_mon:
                ...
                return
            offspring_list = await breed_mons(player_mon["mon_id"], other_mon["mon_id"], str(interaction.user.id))
