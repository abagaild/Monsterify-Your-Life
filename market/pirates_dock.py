IMAGES = [
    "https://example.com/piratesdock1.png",
    "https://example.com/piratesdock2.png",
]
MESSAGES = [
    "Ahoy! Welcome to Pirate's Dock—treasures and adventure await.",
    "Step aboard Pirate's Dock for swashbuckling fun and loot."
]
import discord
import random
import asyncio
...
async def go_fishing_activity(interaction: discord.Interaction, user_id: str):
    ...
    async def effort_callback(inter, effort):
        rolled_mon = randomize_mon({"types": ["Water", "Ice"]}, force_min_types=1)
        rolled_mon["level"] = effort
        modal = TrainerSelectionModal(trainer_callback)
        # Store the rolled mon for later retrieval.
        interaction.data = {"rolled_mon": rolled_mon}
...
async def send_pirates_dock_view(interaction: discord.Interaction, user_id: str):
    ...
    embed = discord.Embed(title="Pirate's Dock", description=self.message, color=discord.Color.gold())
