...
from core.database import update_character_sheet_item, update_mon_data, fetch_one
...
def get_mon(trainer_id: str, mon_name: str) -> Optional[Dict[str, Any]]:
    ...
    Uses the modern database helper.
    """
    query = """
...
async def remove_berry_from_inventory(trainer_sheet: str, berry: str, amount: int = 1) -> bool:
    ...
    Uses the modern inventory update helper by specifying the "Berries" category.
    """
    try:
        result = await update_character_sheet_item(trainer_sheet, berry, -amount, category="Berries")
        return bool(result)
    except Exception as e:
