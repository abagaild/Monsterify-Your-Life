...
import datetime
import json
from core.database import execute_query, fetch_one, get_trainer_currency, addsub_trainer_currency
...
async def roll_generic_shop_items(shop: str, user_id: str, *, category_filter: str = None,
                                  exclude_categories: list = None, count_range: tuple = (2, 5)):
    today = get_today_date()
    # Remove outdated rolls (not matching today's date)
    ...
    if exclude_categories:
        ...
        filtered = [item for item in all_items if not any(
            ex in (item['name'].lower() if isinstance(item, dict) else item.lower()) for ex in exclude_categories)]
    ...
    execute_query("INSERT INTO shop_rolls (shop, user_id, date, items) VALUES (?, ?, ?, ?)",
                  (shop, user_id, today, items_json))
    return items


async def purchase_shop_item(shop: str, user_id: str, trainer_id: int, item_name: str, quantity: int):
    today = get_today_date()
    row = fetch_one("SELECT items FROM shop_rolls WHERE shop=? AND user_id=? AND date=?", (shop, user_id, today))
    if not row:
