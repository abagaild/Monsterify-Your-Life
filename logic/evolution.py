...
from core.database import get_trainers_from_db, cursor

# List of available evolution items
...
def get_full_mons_for_trainer(trainer_id: int) -> List[dict]:
    """
    Returns a list of mon records for the given trainer.
    """
    cursor.execute(
        "SELECT id, mon_name, species1, species2, species3, trainer_id FROM mons WHERE trainer_id = ?",
        (trainer_id,)
    )
...
class EvolutionFlowView(View):
    ...
    Guides the player through the evolution process:
      1. Select a trainer.
      2. Select a mon from that trainer.
      3. Select an evolution item.
      4. If needed, select the target species.
      5. Confirm evolution.
    """
    ...
    class TrainerSelect(Select):
        ...
        def __init__(self, parent_view: 'EvolutionFlowView') -> None:
            ...
            trainers = get_trainers_from_db(parent_view.user_id)
        ...
        async def callback(self, interaction: discord.Interaction) -> None:
            ...
            self.parent_view.selected_trainer = next(
                t for t in get_trainers_from_db(self.parent_view.user_id) if t["id"] == trainer_id
            )
    ...
    class MonSelect(Select):
        """Dropdown for selecting a mon from the chosen trainer."""
        def __init__(self, parent_view: 'EvolutionFlowView') -> None:
            self.parent_view = parent_view
    ...
    class ItemSelect(Select):
        """Dropdown for selecting an evolution item."""
        def __init__(self, parent_view: 'EvolutionFlowView') -> None:
            self.parent_view = parent_view
    ...
    class SpeciesSelect(Select):
        """Dropdown for selecting which species slot to evolve (if applicable)."""
    ...
    class RepairInputModal(Modal, title="Digital Repair Evolution"):
        """Modal for manually entering evolution species when using Digital Repair Mode."""
    ...
    class ConfirmButton(Button):
        """Final button to confirm evolution."""
        def __init__(self, parent_view: 'EvolutionFlowView') -> None:
            self.parent_view = parent_view
            super().__init__(label="Confirm Evolution", style=discord.ButtonStyle.primary,
                             custom_id="confirm_evolution")
