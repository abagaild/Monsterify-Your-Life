...
def get_parent_species(mon: Dict) -> List[str]:
    ...
    """
    return [s for s in [
        (mon.get("species1") or "").strip(),
        (mon.get("species2") or "").strip(),
        (mon.get("species3") or "").strip()
    ] if s]
...
def breed_offspring(parent1: dict, parent2: dict) -> list:
    ...
    for i in range(num_offspring):
        off_species = random.choice(species_pool)
...
async def breed_mons(mon1_id: int, mon2_id: int, user_id: str) -> list:
    ...
    parent1["types"] = [parent1.pop("type1"), parent1.pop("type2"), parent1.pop("type3"), parent1.pop("type4"), parent1.pop("type5")]
    parent1["types"] = [t for t in parent1["types"] if t]
    parent2["types"] = [parent2.pop("type1"), parent2.pop("type2"), parent2.pop("type3"), parent2.pop("type4"), parent2.pop("type5")]
...
# Done Button: Finalizes the registration process.
...
# Offspring Registration View: Presents buttons to register each offspring.
...
# Breed Again Button: Allows the user to restart the breeding process.
...
# Return to Main Menu Button: Lets the user exit the breeding flow.
...
# Farm Result View: Presents post-breeding options after offspring registration.
