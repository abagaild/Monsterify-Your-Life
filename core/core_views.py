...
from core.database import (
    ...
    update_trainer_level, update_mon_level, fetch_mon_by_name
)
...
def create_paginated_trainers_dropdown(trainers: list, placeholder: str, callback, page_size: int = 25) -> PaginatedDropdownView:
    ...
    Each trainer dictionary is expected to have:
      ...
      - "character_name": the trainer's display name.
    ...
    options = [
        discord.SelectOption(
            label=trainer.get("character_name", trainer.get("name", "Unnamed Trainer")),
            value=str(trainer.get("id", "0"))
        )
...
class CompletionRewardModal(Modal, title="Completion Reward"):
    """
    A modal that asks for a trainer/mon name to award levels and items.
    ...
    recipient = TextInput(
        label="Enter trainer/mon name to receive reward",
        placeholder="Trainer/Mon Name",
        required=True
    )

    def __init__(self, subject: str, awarded_levels: int = 0, awarded_items: int = 0):
        super().__init__()
        self.subject = subject
        self.awarded_levels = awarded_levels
        self.awarded_items = awarded_items

    async def on_submit(self, interaction: discord.Interaction):
        await interaction.response.defer(ephemeral=True)

        recipient_name = self.recipient.value.strip()

        # First, try as a trainer:
        trainer = fetch_trainer_by_name(recipient_name)
        if trainer is not None:
            messages = []
            if self.awarded_levels:
                new_level = trainer["level"] + self.awarded_levels
                update_trainer_level(trainer["id"], new_level)
                messages.append(f"{self.awarded_levels} levels awarded (new level: {new_level}).")
            if self.awarded_items:
                # Assuming you have a similar function for updating items
                new_item_count = trainer.get("items", 0) + self.awarded_items
                await update_character_sheet_item(trainer["character_name"], new_item_count)
                messages.append(f"{self.awarded_items} items awarded (new item count: {new_item_count}).")
            if not messages:
                messages.append("No rewards were set to be awarded.")
            await interaction.followup.send(
                f"Reward processed for trainer '{trainer['character_name']}' for completing '{self.subject}':\n" +
                "\n".join(messages),
                ephemeral=True
            )
            return
        ...
        # Then, try as a mon:
        mon = fetch_mon_by_name(recipient_name)  # Adjust as needed for your lookup logic.
        if mon is not None:
            messages = []
            if self.awarded_levels:
                new_level = mon["level"] + self.awarded_levels
                update_mon_level(mon["id"], new_level)
                messages.append(f"{self.awarded_levels} levels awarded (new level: {new_level}).")
            if self.awarded_items:
                new_item_count = mon.get("items", 0) + self.awarded_items
                trainer_name = mon["trainer_name"]
                trainer = fetch_trainer_by_name(mon["trainer_name"])
                await update_character_sheet_item(trainer_name, self.awarded_items, new_item_count)
                messages.append(f"{self.awarded_items} items awarded (new item count: {new_item_count}).")
            if not messages:
                messages.append("No rewards were set to be awarded.")
            await interaction.followup.send(
                f"Reward processed for mon '{mon['name']}' for completing '{self.subject}':\n" +
                "\n".join(messages),
                ephemeral=True
            )
            return
        ...
        await interaction.followup.send(
            f"Recipient '{recipient_name}' not found. Unable to update reward.",
            ephemeral=True
        )

async def send_player_message(interaction: discord.Interaction, content: str):
    """
    ...
    if channel is not None:
        ...
        # Fallback: use the interaction response if channel is not available.
