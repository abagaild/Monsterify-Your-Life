...
from core.database import get_trainers_from_database, get_other_trainers_from_db
from views.add_trainer import AddTrainerEmbedView
from views.trainer import PaginatedTrainersView
...
from core.config import (
    ...
    game_corner_channel_id, boss_channel_id
)
from views.trades import TradeMonsModal, TradeItemsModal
from views.add_trainer import AddTrainerEmbedView
...
class MainMenuView(View):
    ...
    @discord.ui.button(label="🌇 Visit Town", style=discord.ButtonStyle.primary, custom_id="menu_visit_town", row=0)
    ...
    @discord.ui.button(label="👤 Character", style=discord.ButtonStyle.primary, custom_id="menu_character", row=0)
    ...
    @discord.ui.button(label="⚔️ Boss", style=discord.ButtonStyle.danger, custom_id="menu_boss", row=0)
...
class TownMenuView(View):
    def __init__(self):
        ...
        self.main_embed = discord.Embed(
            title="Adventure Awaits!",
            description="The town bustles with the adventures of your friends. Choose an option below to begin your journey.",
            color=discord.Color.green()
        )
        self.main_embed.set_image(url="https://i.imgur.com/SZkGh26.png")
        self.main_embed.set_footer(text="Where shall you go first? There are endless possibilities.")
    ...
    async def apothecary(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.defer()
        from market.apothecary import send_apothocary_overall_view
    ...
    async def bakery(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.defer()
        from market.bakery import send_bakery_view
    ...
    async def witch_hut(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.defer()
        from market.witchs_hut import send_witchs_hut_view
    ...
    async def antique_store(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.defer()
        from market.antiques import send_antique_overall_view
    ...
    async def adoption_center(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.defer()
        from market.adoption_center import send_adoption_center_view
        await send_adoption_center_view(interaction, str(interaction.user.id), get_trainers_from_database(str(interaction.user.id)))
    ...
    async def pirate_dock(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.defer()
        from market.pirates_dock import send_pirates_dock_view
    ...
    async def megamart(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.defer()
        from market.megamart import send_megamart_view
    ...
    async def farm(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.defer()
        from market.farm import send_farm_view
    ...
    async def nursery(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.defer()
        from market.nursery import send_nursery_view
...
class CharacterMenuView(View):
    ...
    async def view_my_characters(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        view = PaginatedTrainersView(trainers, editable=True, user_id=user_id)
    ...
    @discord.ui.button(label="Trade Pokemon", style=discord.ButtonStyle.primary, custom_id="trade_pokemon", row=1)
    async def trade_pokemon_button(self, button: discord.ui.Button, interaction: discord.Interaction):
        modal = TradeMonsModal()
        await interaction.response.send_modal(modal)

    @discord.ui.button(label="Trade Items", style=discord.ButtonStyle.primary, custom_id="trade_items", row=1)
    async def trade_items_button(self, button: discord.ui.Button, interaction: discord.Interaction):
        modal = TradeItemsModal()
        await interaction.response.send_modal(modal)
